...
export const SalaryChart = ({ balancedata }) => {
    ...
    const latestAvailable = chartData.at(-1)?.AvailableAmount ?? 0
    const previousAvailable = chartData.at(-2)?.AvailableAmount ?? latestAvailable
    let trendingUp = 0

    if (chartData.length > 1 && previousAvailable !== 0) {
        const difference = latestAvailable - previousAvailable
        trendingUp = Math.round((difference * 100) / previousAvailable)
    }
    return (
